...
const (
	...
	PROJECT_ID  = "pure-heuristic-225722"
	BT_INSTANCE = "around-post"
	// elastic search url
	ES_URL = "http://35.229.80.215:9200"
...
// save image to GCS
// io.Reader r: file
// bucketName: bucket on GCS
// objectName: the object name that will be created on GCS, which is used to store file
func saveToGCS(r io.Reader, bucketName, objectName string) (*storage.ObjectAttrs, error) {
	ctx := context.Background()

	// Creates a client.
	// client, err := storage.NewClient(ctx)
	client, err := storage.NewClient(ctx, option.WithCredentialsFile("Around-7b2948b41571.json"))
	if err != nil {
		return nil, err
	}
	...
	// new an object on GCS with objectName
	object := bucket.Object(objectName)
	// writer for writing to the object
	wc := object.NewWriter(ctx)
	// copy the content in r(the file) into the object (write to the object)
...
/* func saveToBigTable(p *Post, id string) {
	ctx := context.Background()
	bt_client, err := bigtable.NewClient(ctx, "pure-heuristic-225722", "around-post", option.WithCredentialsFile("Around-7b2948b41571.json"))
	if err != nil {
		panic(err)
		return
	}
